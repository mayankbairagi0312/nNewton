...
inline const nColor nColor::White{ 1.0f, 1.0f, 1.0f, 1.0f };
inline const nColor nColor::Red{ 1.0f, 0.0f, 0.0f, 1.0f };
inline const nColor nColor::Green{ 0.0f, 1.0f, 0.0f, 1.0f };
inline const nColor nColor::Blue{ 0.0f, 0.0f, 1.0f, 1.0f };
inline const nColor nColor::Yellow{ 1.0f, 1.0f, 0.0f, 1.0f };
inline const nColor nColor::Cyan{ 0.0f, 1.0f, 1.0f, 1.0f };
inline const nColor nColor::Magenta{ 1.0f, 0.0f, 1.0f, 1.0f };
inline const nColor nColor::Invalid{ -1.0f,-1.0f,-1.0f,-1.0f };

inline nColor::nColor(int index) {
    if (index < 0 || index > 255) { *this = Invalid; return; }
    if (index < 16) {
        ...
        r = ansi[index][0]; g = ansi[index][1]; b = ansi[index][2]; a = 1.0f;
    }
    else if (index < 232) {
        int idx = index - 16;
        int red = (idx / 36) % 6, green = (idx / 6) % 6, blue = idx % 6;
